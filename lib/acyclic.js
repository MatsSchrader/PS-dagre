...
function dfsFAS(g, oldGraph) {
  ...
  function dfsDynamic(v) {
    ...
    g.outEdges(v).forEach(e => {
      if (Object.hasOwn(stack, e.w) ||
        (oldGraph.node(v)?.rank > oldGraph.node(e.w)?.rank) && isReachedFromStartWithoutEdge(g, e.w, e)) {
        fas.push(e);
      } else {
        dfsDynamic(e.w);
      }
  ...
  g.sources().forEach(dfsFn);
  g.nodes().forEach(dfsFn);
...
function isReachedFromStartWithoutEdge(g, node, ignoreEdge) {
  ...
  function reverseDfs(v) {
    ...
    for (let e of g.inEdges(v)){
      ...
      && !Object.hasOwn(localVisited, e.v)) {
        if (reverseDfs(e.v)) {
          return true;
        }
      }
