 ...
 */
function verticalAlignment(g, layering, conflicts, neighborFn, corePath = []) {
  let root = {},
    align = {},
    pos = {};
    ...
    modifiedLayer.forEach(v => {
      ...
      if (ws.length) {
        if (corePath.includes(v)) {
          ws = ws.filter((w) =>{
            return isInCorePath(w, g, corePath);
          });
        }
...
function positionX(g, corePath) {
  ...
  ["u", "d"].forEach(vert => {
    ...
    ["l", "r"].forEach(horiz => {
      ...
      let align = verticalAlignment(g, adjustedLayering, conflicts, neighborFn, corePath);
...
function isInCorePath(v, g, corePath) {
  const edge = g.node(v)?.edgeObj;
  if (!edge || g.node(v).edgeLabel.reversed) return false;
