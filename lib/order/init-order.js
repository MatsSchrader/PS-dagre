...
function initOrder(g, oldGraph) {
  ...
  /**
   * Compares two nodes based on the positions of their targets and source in the oldGraph.
   * @param a first node to compare
   * @param b second node to compare
   * @returns 0 if equal or not comparable, negative if a should come before b, positive if b should come before a
   */
  ...
  /**
   * Recursively finds the next target node in the old graph for a given edge node.
   * This function looks if v is in oldGraph, if not, it checks the successors of v recursively.
   * @param v the starting node to search for in the old graph
   * @returns the next target node in the old graph, usually will never be null since all graphs end in 'End'
   */
  ...
  /**
   * Groups reversed edge pairs in the successors array.
   * Looks at the successors - if they are dummy edge nodes with an edgeObject: {v: , w: }, compares the targets (source is identical since successors are from the same source node)
   * If the targets are the same, moves the later edge to the position after the first edge in the successors array.
   * @param successors
   */
  function groupReversedEdgePairs(successors) {
    for (let i = 0; i < successors.length; i++) {
      for (let j = i + 1; j < successors.length; j++) {
        let nameI = successors[i];
        let nameJ = successors[j];
        ...
        if (nodeI.dummy === "edge" && nodeJ.dummy === "edge" &&
          ...
          successors.splice(i + 1, 0, nameJ);
          j = successors.length; // break inner loop since there can be only one reversed version of edge i
        }
      }
    }
  }
